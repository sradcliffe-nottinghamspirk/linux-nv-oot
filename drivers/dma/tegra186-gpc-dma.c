					   ...
					   struct dma_tx_state *txstate)
{
	...
	if (ret == DMA_COMPLETE)
		return ret;
	if (tdc->status == TEGRA_DMA_PAUSED)
		ret = DMA_PAUSED;
					   ...
					   struct of_dma *ofdma)
{
	...
	if (tdma->channel_safety_map) {
		if (dma_spec->args_count != 3)
			return NULL;

		chan_id = dma_spec->args[1];
		if (chan_id >= tdma->nr_channels)
			return NULL;
		chan = dma_get_slave_channel(
			&tdma->channels[chan_id].vc.chan);
		if (!chan)
			return NULL;
	} else {
...
static int tegra_dma_probe(struct platform_device *pdev)
{
	...
	tdma->channel_safety_map = of_property_read_bool(pdev->dev.of_node,
				"dma-channel-safety-map");

	INIT_LIST_HEAD(&tdma->dma_dev.channels);
	for (i = 0; i < tdma->nr_channels; i++) {
		struct tegra_dma_channel *tdc = &tdma->channels[i];

		tdc->irq = platform_get_irq(pdev, i + rsvd_chan_count);
		if (tdc->irq < 0)
			return tdc->irq;
